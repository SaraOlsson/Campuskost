    ...
    const onDragEnd = result => {
        ...
        if(destination.droppableId.substring(0, 4) === 'list') {
          ...
          listsRef.get().then(function(doc) {
            ...
            //let original_list = Array.from(lists_by_user);
            //original_list[list_index] = data;
            //setLists_by_user(original_list);
            ...
            //let newArr = [...lists_by_user]; // copying the old datas array
            // let newListObj = data; // newArr[list_index];
            // newListObj.recipes = updated_recipes
            //newArr[list_index] = data; // replace e.target.value with whatever you want to change it to

            //setLists_by_user(newArr);
            // history.push("/lists" );
              ...
              <div ref={provided.innerRef} >
                ...
                <div
                  className={isDragging ? classes.displayNone : null}>
                    <ListContainer key={i}
                       recipemap={list_doc.recipes}
                       listname={list_doc.listname}
                       createdby={list_doc.created_by}
